...
for k=1:len_xtrain
    ...
    for i=1:M
        t1=zeros(1,I);
        for j=1:I
           t1(j)=exp(-(xtrain(k,j)-mu(i,j))^2/(2*sigma(i,j)^2));
        end  
        z(i)=prod(t1); 
        a(i)=c(i)*z(i);   
    end
    ...
    for i1=1:M   %分别计算梯度并存入***_df矩阵
        c_df(i1,:,k)=(fls_ff-ytrain(k))*z(i1)/b;
        for i2=1:I
            mu_df(i1,i2,k)=(fls_ff-ytrain(k))/b*(c(i1)-fls_ff)*z(i1)*(xtrain(k,i2)-mu(i1,i2))/sigma(i1,i2)^2;
            sigma_df(i1,i2,k)=(fls_ff-ytrain(k))/b*(c(i1)-fls_ff)*z(i1)*((xtrain(k,i2)-mu(i1,i2))^2)/sigma(i1,i2)^3;
        end
    end  
    %k=k+1;
end
...
mu_end=zeros(M,I);
sigma_end=zeros(M,I);
Err=sqrt(sum((fls_f-ytrain').^2)/length(fls_f));
for i=1:len_xtrain
