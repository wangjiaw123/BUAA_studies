...
Error=0.000001;
Beta=0.6;
Gama=0.0000005;%设置学习率,小于0.005效果好一些
n=length(inputdata(1,:));
%sigma_up=0.3*ones(n*K,1);
sigma_up=0.3*rand(n*K,1);
Sigma_up=sigma_up;
sigma_low=0.1*ones(n*K,1);
sigma_low=0.1*rand(n*K,1);
Sigma_low=sigma_low;
c=1*rand(n*K,1);
ccc=c;
W=2*rand(4*K,8);
...
%% 更新参数b    
      for i2=1:4*K %用到了公式28，35，36，37，38
          ...
          a2=0;
          for k1=1:i
              delta_f111=delta*TsaveCYCLIC_o(a1,1,k1)*(1-(compute_tanh(TsaveCYCLIC_c(a1,1,k1)))^2)*TsaveCYCLIC_c(a1,1,k1)*TsaveCYCLIC_f(a1,1,k1)*(1-TsaveCYCLIC_f(a1,1,k1));
              a2=a2+delta_f111;  
          end
          b(i2,1)=b(i2,1)-Gama*a2;
...
%           end
          ...
          b2=0;
          for k2=1:i
          delta_i111=delta*TsaveCYCLIC_o(b1,2,k2)*(1-(compute_tanh(TsaveCYCLIC_c(b1,2,k2)))^2)*TsaveCYCLIC_ct(b1,2,k2)*TsaveCYCLIC_i(b1,2,k2)*(1-TsaveCYCLIC_i(b1,2,k2));
          b2=b2+delta_i111;  
          end                 
          b(i2,2)=b(i2,2)-Gama*b2;   
...
%           end
          ...
          c2=0;
          for k3=1:i
              delta_ct111=delta*TsaveCYCLIC_o(c1,3,k3)*(1-(compute_tanh(TsaveCYCLIC_c(c1,3,k3)))^2)*TsaveCYCLIC_i(c1,3,k3)*(1-(compute_tanh(TsaveCYCLIC_ct(c1,3,k3)))^2);
              c2=c2+delta_ct111;
          end
          b(i2,3)=b(i2,3)-Gama* c2;   
...
%           end
          ...
          d2=0;
          for k4=1:i
              delta_o111=delta*compute_tanh(TsaveCYCLIC_ct(d1,4,k4))*TsaveCYCLIC_o(d1,4,k4)*(1-TsaveCYCLIC_o(d1,4,k4));
              d2=d2+delta_o111;
          end
          b(i2,4)=b(i2,4)-Gama*d2;  
      end
...
%% 更新参数W
      for i3=1:4*K
          ...
          W(i3,1)=W(i3,1)-Gama*delta_f*Tsave_Layer3F(e11,xc1,i);    
...
%           end
          ...
          W(i3,3)=W(i3,3)-Gama*delta_i22*Tsave_Layer3F(f11,xc1,i);   
...
%           end
          ...
          for k2=2:i
              ...
              H=Tsave_Layer4h(b11,2,k2-1);                    
              b22=b22+delta_i22*Tsave_Layer4h(b11,2,k2)*H;
          end
          W(i3,4)=W(i3,4)-Gama*b22;
...
%           end
          ...
          W(i3,5)=W(i3,5)-Gama*delta_ct22*Tsave_Layer3F(g11,xc1,i);  
...
%           end
          ...
          W(i3,7)=W(i3,7)-Gama*delta_o22*Tsave_Layer3F(h11,xc1,i);  
