...
SAVEserial_total_time = zeros(RULE_NUMBER,Monte_carlo);
SAVE_serial_M1    = zeros(RULE_NUMBER*RULE_WIDTH,n,RULE_NUMBER,Monte_carlo);
SAVE_serial_M2    = zeros(RULE_NUMBER*RULE_WIDTH,n,RULE_NUMBER,Monte_carlo);
SAVE_serial_sigma = zeros(RULE_NUMBER*RULE_WIDTH,n,RULE_NUMBER,Monte_carlo);
SAVE_serial_C    = zeros(RULE_NUMBER*RULE_WIDTH,n+1,RULE_NUMBER,Monte_carlo);
SAVE_serial_S    = zeros(RULE_NUMBER*RULE_WIDTH,n+1,RULE_NUMBER,Monte_carlo);
SAVE_serial_Ytest = zeros(K,RULE_NUMBER,Monte_carlo);
SAVE_serial_Ypridect = zeros(K,RULE_NUMBER,Monte_carlo);
SAVE_RMSE_serial = zeros(Tmax,RULE_NUMBER,Monte_carlo);

DATA_SIZE=1;
Zero1 = zeros(CORE_NUMBER,DATA_SIZE,RULE_NUMBER,Monte_carlo);  %1维是分配的核心数的个数，2维是数据规模数的个数，3维是规则数的个数,4维是Monte_carlo的次数
...
SAVE_RMSE_parallel = zeros(CORE_NUMBER,Tmax,DATA_SIZE,RULE_NUMBER,Monte_carlo);   %1维是分配的核心数的个数，2维是Tmax(这一维用于存储RMSE),3维是数据规模数的个数，4维是规则数的个数

SAVE_parallel_M1    = zeros(CORE_NUMBER,RULE_NUMBER*RULE_WIDTH,n,DATA_SIZE,RULE_NUMBER,Monte_carlo);
SAVE_parallel_M2    = zeros(CORE_NUMBER,RULE_NUMBER*RULE_WIDTH,n,DATA_SIZE,RULE_NUMBER,Monte_carlo);
SAVE_parallel_sigma = zeros(CORE_NUMBER,RULE_NUMBER*RULE_WIDTH,n,DATA_SIZE,RULE_NUMBER,Monte_carlo);
SAVE_parallel_C    = zeros(CORE_NUMBER,RULE_NUMBER*RULE_WIDTH,n+1,DATA_SIZE,RULE_NUMBER,Monte_carlo);
SAVE_parallel_S    = zeros(CORE_NUMBER,RULE_NUMBER*RULE_WIDTH,n+1,DATA_SIZE,RULE_NUMBER,Monte_carlo);
SAVE_parallel_Ytest = zeros(CORE_NUMBER,K,DATA_SIZE,DATA_SIZE,RULE_NUMBER,Monte_carlo);
SAVE_parallel_Ypridect = zeros(CORE_NUMBER,K,DATA_SIZE,RULE_NUMBER,Monte_carlo);
...
%           legend('预测值','实际值')      
    ...
    SAVEserial_total_time(rule_num,MC)      = serial_total_time;   
    SAVE_serial_M1(1:length(M1(:,1)),:,rule_num,MC)    = M1;
    SAVE_serial_M2(1:length(M2(:,1)),:,rule_num,MC)     = M2;
    SAVE_serial_sigma(1:length(sigma(:,1)),:,rule_num,MC)  = sigma;
    SAVE_serial_C(1:length(C(:,1)),:,rule_num,MC)     = C;
    SAVE_serial_S(1:length(S(:,1)),:,rule_num,MC)    = S;
    SAVE_serial_Yrain(1:length(Ytrain(:,1)),rule_num,MC) = Ytrain;
    SAVE_serial_Ypridect(1:length(Rs_serial),rule_num,MC) = Rs_serial; 
    SAVE_RMSE_serial(:,rule_num,MC) =  RMSE_serial_save;
end    

%% 并行程序部分
for core_num=6:3:CORE_NUMBER %   %核心数的循环
          ...
          %% 训练数据随机重排
           ...
           parfor i22=1:core_num
                  ...
                  [M1_help(:,:,i22),M2_help(:,:,i22),C_help(:,:,i22),S_help(:,:,i22),sigma_help(:,:,i22)]...
                            =train_tsk_type2(XX_train(:,:,i22),YY_train(:,:,i22),M1,M2,sigma,C,S,1*alpha*core_num);%2
           end
           ttime21=toc;
...
%           legend('预测值','实际值')
    ...
    %%
    SAVE_parallel_M1(core_num,1:length(M1_save(:,1,wz)),:,rule_num,MC)    = M1_save(:,:,wz);
    SAVE_parallel_M2(core_num,1:length(M2_save(:,1,wz)),:,rule_num,MC)    = M2_save(:,:,wz);
    SAVE_parallel_sigma(core_num,1:length(sigma_save(:,1,wz)),:,rule_num,MC) = sigma_save(:,:,wz);
    SAVE_parallel_C(core_num,1:length(C_save(:,1,wz)),:,rule_num,MC)      = C_save(:,:,wz);
    SAVE_parallel_S(core_num,1:length(S_save(:,1,wz)),:,rule_num,MC)      = S_save(:,:,wz);
    SAVE_parallel_Ytrain(core_num,1:length(Ytrain),DATA_SIZE,rule_num,MC)       = Ytrain;
    SAVE_parallel_Ypridect(core_num,1:length(Rs),DATA_SIZE,rule_num,MC) = Rs;   

    %% 记录并行总时间，并行内部时间，串行时间，计算加速比、并行效率,串行比例
    SAVEparallel_total_time(core_num,i,rule_num,MC)    = parallel_total_time;      
    SAVEparallel_interior_time(core_num,i,rule_num,MC) = parallel_interior_time;         
    SAVEspeedUP_ratio(core_num,i,rule_num,MC)          = SAVEserial_total_time(rule_num,i)/parallel_total_time;
    SAVEparallel_efficiency(core_num,i,rule_num,MC)    = SAVEserial_total_time(rule_num,i)/(core_num*parallel_total_time);
    SAVEserial_proportion(core_num,i,rule_num,MC)     = (parallel_total_time - parallel_interior_time)/parallel_total_time;   
    SAVE_RMSE_parallel(core_num,:,i,rule_num,MC) = RMSE_parallel_save;
...
%averTSKspeedup=zeros(size(SAVEspeedUP_ratio(:,:,:,1)));CORE_NUMBER,DATA_SIZE,RULE_NUMBER
averTSKspeedup=zeros(CORE_NUMBER,DATA_SIZE,RULE_NUMBER);
for moncarl=1:Monte_carlo
    averTSKspeedup=averTSKspeedup+SAVEspeedUP_ratio(:,:,:,moncarl);
end
averTSKspeedup=averTSKspeedup./Monte_carlo;
...
save('averageTSKspeedupRatio','averTSKspeedup')