...
RULR_NUMBER = 2;
RULE_WIDTH = 10;
CORE_NUMBER = 6;      %查看并行部分，核心数的使用是递减的！
%CORE_NUMBER_WIDTH=1;
DATA_SIZE = 1;        %max_select_train/data_size=7                              
Tmax = 40;                                         %设置模型训练中的最大迭代次数

Zero1 = zeros(CORE_NUMBER,DATA_SIZE,RULR_NUMBER);   %1维是分配的核心数的个数，2维是数据规模数的个数，3维是规则数的个数
...
SAVE_RMSE__parallel_G = zeros(CORE_NUMBER,Tmax,DATA_SIZE,RULR_NUMBER);   %1维是分配的核心数的个数，2维是Tmax(这一维用于存储RMSE),3维是数据规模数的个数，4维是规则数的个数
...
SAVEserial_total_time_C = zeros(RULR_NUMBER,DATA_SIZE);        %1维是规则数的个数，2维是数据规模数的个数
SAVE_RMSE_serial_F    = zeros(RULR_NUMBER,Tmax,DATA_SIZE);     %维是规则数的个数，2维是Tmax(这一维用于存储RMSE),3维是数据规模数的个数
...
for i=1:DATA_SIZE                   %任务规模的循环
   %% 时间记录参数初始化
    ...
    while  (T1 < Tmax) && (RMSE  >= setRMSE_accept )%|| isnan(RMSE)==1 
...
%% 并行程序部分
for core_num=CORE_NUMBER:-1:5     %核心数的循环
...
%            c2 = g_c2;
    ...
    SAVE_parallel(core_num,1:length(M2_save(:,1,wz)),:,i,rule_num)    = M2_save(:,:,wz);
...
save('SAVEdata_parallel_parameter','SAVE_parallel_M1','SAVE_parallel','SAVE_parallel_sigma',...
