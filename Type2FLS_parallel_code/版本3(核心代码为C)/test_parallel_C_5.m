...
%CORE_NUMBER_WIDTH=1;
DATA_SIZE = 2;%7  %max_select_train/data_size=7                              
Tmax = 20;  %40                                       %设置模型训练中的最大迭代次数
...
for i=1:DATA_SIZE                   %任务规模的循环
   %% 时间记录参数初始化
    ...
    while  T1 <Tmax %&& RMSE  >=setRMSE_accept
           ...
           [M1,M2,c1,c2,sigma]=Train_sfls_type2_C(X_train,Y_train,M1,M2,c1,c2,sigma,(1-T1/Tmax+0.1)*alpha);
...
% 
% 
% for core_num=CORE_NUMBER %   %核心数的循环
%     
%     parpool(core_num);          %在MATLAB中开启核心数为core_num的并行计算池  
%     poolobj = gcp('nocreate')
%      addAttachedFiles(poolobj,{'C:\Users\wangjiawen\Desktop\版本3(核心代码为C)\Train_sfls_type2_C.mexw64',...
%                  'C:\Users\wangjiawen\Desktop\版本3(核心代码为C)\sfls_type2_C.mexw64'})
% %     addAttachedFiles(poolobj,{'F:\王佳文\重要文件\交给老师的文件\二型模糊逻辑系统的并行实现\2型模糊逻辑系统---并行程序\程序(正确的)\版本3(核心代码为C)\Train_sfls_type2_C.mexw64',...
% %                 'F:\王佳文\重要文件\交给老师的文件\二型模糊逻辑系统的并行实现\2型模糊逻辑系统---并行程序\程序(正确的)\版本3(核心代码为C)\sfls_type2_C.mexw64'})   
% %             
% for rule_num=1:RULE_NUMBER
%     
%     N=RULE(rule_num);                     %规则数目的循环
