...
%% 全局变量
n = gpuArray(4);%模糊规则前件个数为4
alpha =gpuArray(0.4);%KM时取0.4
setRMSE_accept=gpuArray(0.04);  %设置可接受的误差
%% 此处是最终要保存的数值矩阵，多维数组一开始大小不要分配错了!
max_select_train = gpuArray(2100);
data_size = gpuArray(100);%1000
RULE=gpuArray([20,60]);%[20,60]
RULE_NUMBER = gpuArray(length(RULE));%10
RULE_MAX = gpuArray(max(RULE));
Monte_carlo=gpuArray(1);%蒙特卡罗实验20
CORE_NUMBER =gpuArray(6);%2:2:24      %查看并行部分，核心数的使用是递减的！
%CORE_NUMBER_WIDTH=1;
DATA_SIZE = gpuArray(1);%7  %max_select_train/data_size=7                              
Tmax = gpuArray(10);  %40                                       %设置模型训练中的最大迭代次数
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
Zero1 = gpuArray(zeros(CORE_NUMBER,DATA_SIZE,RULE_NUMBER,Monte_carlo));  %1维是分配的核心数的个数，2维是数据规模数的个数，3维是规则数的个数
...
%     c2 = gpuArray(c1+1);       
    ...
    R=sfls_type2(X_train,M1,M2,sigma,c1,c2); %R(i)=1/2(R1+R2)
...
for core_num=CORE_NUMBER  %2:2:CORE_NUMBER %   %核心数的循环
