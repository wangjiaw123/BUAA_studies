function [ XX,fvalue ] = approximate_method( c,d,A,b )
...
lambda2=(c*XX)/(d*XX);
while ~(abs(lambda1-lambda2) <= 10e-4)
    X = XX;
    lambda1 = (c*X)/(d*X);
    [X1star,fval4,exitflag4,outpt4,lama4] = linprog(-d,A,b,c-lambda1*d,0,zeros(length(c),1));
    if d*X == d*X1star
       [X1star,fval5,exitflag5,outpt5,lama5] = linprog(d,A,b,c-lambda1*d,0,zeros(length(c),1));
    end
    X2star=(X1star+X)/2;
