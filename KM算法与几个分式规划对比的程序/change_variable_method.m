...
if denominator_min >= 0      
    ...
    output_x = x11(1:length(c1))./x11(length(c11));
     %output_value = -fval11;
    output_value = (c1*output_x+c0)/(d1*output_x+d0);
...
%      [x11,fval11,exitflag111,output111,lambda111] = linprog(-c11,A,zeros(m,1),Aeq,beq,zeros(length(c11),1));
%      [x22,fval22,exitflag222,output222,lambda222] = linprog(c22,A,zeros(m,1),-Aeq,beq,zeros(length(c22),1));
%      x33 = [x11 x22];
...
%      output_x = outx(1:length(c11)-1)./outx(length(c11));
     ...
     [x11,fval11,exitflag111,output111,lambda111] = linprog(-c11,A,zeros(m,1),Aeq,beq,zeros(length(c11),1));
     output_x1 = x11(1:length(x11)-1)./x11(length(x11));
     output_value1 = (c1'*output_x1+c0)/(d1'*output_x1+d0);
     [x22,fval22,exitflag222,output222,lambda222] = linprog(c22,A,zeros(m,1),-Aeq,beq,zeros(length(c22),1));
     output_x2 = x22(1:length(x22)-1)./x22(length(x22));
     output_value2 = (c1'*output_x2+c0)/(d1'*output_x2+d0);
     x33 = [x11 x22];
     output_value3 = [output_value1 output_value2];
     la1 = find(output_value3 == max(output_value3));
     output_x = x33(la1);
     output_value = output_value3(la1);
end
